...
function existingIncludePaths() {
  return includePaths.filter((relativePath) => fs.existsSync(path.join(publishDir, relativePath)));
}
...
function main() {
  ...
  ensureGitIdentity();
  run("git", ["add", ...existingIncludePaths()], { cwd: publishDir });
